...
\begin{abstract}
We describe the integration-by-parts (IBP) technique used to improve the numerical evaluation of Fermi--Dirac integrals for baryon number susceptibilities $\chi_n^B$ and the entropy density temperature derivative $ds/dT|_\mu$ at low temperatures in the hadron resonance gas model.
...
For $ds/dT|_\mu$, we derive a double-IBP formula involving the Fermi entropy kernel $\sigma(x)$ that achieves $\sim 2 \times 10^{-4}$ relative accuracy at $T = 1$~$\mu$eV, and explain why the $\chi_2$-type analytical extraction of the $T=0$ value is fundamentally unavailable for temperature derivatives.
\end{abstract}
...
This is the same $\sigma$ kernel used in the entropy density $s = (g/2\pi^2)\int p^2\,\sigma\,dp$, so the quadrature accuracy is comparable to that of~$s$ itself: $\sim 2 \times 10^{-4}$ relative error at $T = 1$~$\mu$eV.
