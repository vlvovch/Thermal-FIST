  ...
  */
  ...
  public:
    ...
    virtual double pf(double x) const { 
      return x * x * m_fpi * m_fpi / 4. * (1 - m_mass * m_mass / x / x) * (1 - m_mass * m_mass / x / x) * pow(xMath::GeVtoifm(), 3); 
    }
  ...
  public:
    ...
    bool IsBECPhase() const { return m_isBEC; }
    ...
    {
    public:
      BroydenEquationsEMMTBEC(const EffectiveMassModel* model, double MuBEC) : BroydenEquations(), m_EMM(model), m_MuBEC(MuBEC) { m_N = 1; }
    ...
    {
    public:
      BroydenEquationsEMMMeffConstrained(const EffectiveMassModel* model) : BroydenEquations(), m_EMM(model) { m_N = 1; }
      std::vector<double> Equations(const std::vector<double>& x);
    private:
      const EffectiveMassModel* m_EMM;
    };
