...
//        fDistribution = NULL;
    ...
    double GetModeldNdp(double p) const {
        ...
        if (fDistribution==NULL) return 0.;// || events==0) return 0.;
        else return fDistribution->dndp(p);// * GetMean();
    }
...
//    double GetdNdpError(double p) const {
//        if (fDistribution==NULL) return 0.;
//        else return fDistribution->dndp(p) * GetMeanError();
//    }
...
//            tmin = min(tmin, ty);
//            tmax = max(tmax, ty);
            //plotDistr->graph(1)->addData(tx, ty);
            //qDebug() << tx << " " << ty;
        }
    }
