    ...
    QWidget(parent)
{
    ...
    *model = *modelop;
    ...
    plotDistr->addGraph();
    //plotDistr->graph(0)->setName(comboDistr->currentText());
    ...
    //dataLayv2->addWidget(plotDistr,1);
   ...
   // QGroupBox *grEditor = new QGroupBox(tr("Editor"));
    ...
    //radioCE = new QRadioButton(tr("CE-HRG"));
    //radioIdealCanonStrangeness = new QRadioButton(tr("S-CE-HRG"));
    //radioIdealCanonCharm = new QRadioButton(tr("C-CE-HRG"));
    ...
    //connect(radioCE, SIGNAL(clicked()), this, SLOT(modelChanged()));
    //connect(radioIdealCanonStrangeness, SIGNAL(clicked()), this, SLOT(modelChanged()));
    //connect(radioIdealCanonCharm, SIGNAL(clicked()), this, SLOT(modelChanged()));

    layModels->addWidget(radIdeal);
    layModels->addWidget(radEVD);
		layModels->addWidget(radQVDW);
    //layModels->addWidget(radioCE);
    ...
    layFlags->addWidget(checkBratio);
		...
		//leFilePath->setMaximumWidth(200);
		...
		//layFilePath->addWidget(labelFilePath);
    ...
    DataEditLay->addWidget(splitter);
...
void EventGeneratorTab::replot2D(const QVector<double> &xv, const QVector<double> &yv, const QVector<double> &zv, int index)
{
    plot2D->xAxis->setRange(-3., 3.);
    if (spectra->fDistributionType==1) plot2D->xAxis->setRange(-3. - spectra->fEtaMax, 3. + spectra->fEtaMax);
    plot2D->yAxis->setRange( 0., 2.);
...
void EventGeneratorTab::updateProgress() {
    ...
    if (index>=0 && index<3 && id<spectra->fParticles.size()) {
        replot(x1,y1,y1err,x2,y2,index);
    }

    if (index==3 && id<spectra->fParticles.size()) {
        replot2D(x1,y1,z1,index);
    }
